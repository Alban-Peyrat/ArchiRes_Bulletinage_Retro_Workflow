...
Les scripts dans  ([*scripts*](./scripts/)) sont des Office Scripts (donc Typescript), maintenus dans des fichiers Javascripts qui sont transformés en Office Scripts via [un flux Power Automate de "conversion"](./js_to_osts.md).
...
Pour mettre à jour la liste des localisations dans Koha, utiliser la requête SQL ci-dessous et remplacer la déclaration de la variable `kohaLocations` dans [*scripts/2_main_loop.js*](./scripts/2_main_loop.js) :
...
* **Répeter cette action 4 fois supplémentaire en changeant le script par celui qui suit**
* 6) _Set variable_ :
  * _Name_ : _MailOutput_
...
* 7) _Set variable_ :
  ...
  * _Value_ : `concat('Le fichier ', triggerBody()?['{FilenameWithExtension}'],' n a pas pu être traité.')`
    * `triggerBody()?['{FilenameWithExtension}']` = _Nom de fichier avec l'extension_ de la première étape
  * _Settings_ → _Run after_ : tous les _Run script from SharePoint library_ sauf en cas de succès
* 8) _Office 365 Outlook : Send an email (V2)_ :
  * _To_ : destinaires
